...
def init_db() -> None:
    with get_connection() as conn:
        ...
        if "media" not in ads_columns: # Safely handle the very first time setup
            conn.execute("ALTER TABLE ads ADD COLUMN media TEXT NOT NULL DEFAULT ''")
        ...
        if "discount" not in ads_columns:
            conn.execute("ALTER TABLE ads ADD COLUMN discount REAL NOT NULL DEFAULT 0")
