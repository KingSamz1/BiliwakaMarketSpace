...
from utils import apply_theme, init_session_defaults, render_footer, render_carousel

st.set_page_config(page_title="Biliwaka MarketSpace", page_icon="🛒", layout="wide")
init_session_defaults()
init_db()

# ── Reliable Navigation via Session State (no st.switch_page needed) ──
...
# ── Custom Topbar ──
try:
    from utils import render_topbar
    render_topbar()
except:
    pass
        ...
        if target_file:
            try:
                with open(target_file, "r", encoding="utf-8") as f:
                    code = f.read()
                exec(code, {"__name__": "__main__", "st": st, "Path": Path, "go_to": go_to})
